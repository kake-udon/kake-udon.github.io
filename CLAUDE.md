...
- **Web Push通知**（ロードマップ6）：表紙画面に通知トグルを追加、毎日18時（JST）にお気に入りチームの当日結果と次戦予定をダイジェスト通知。バックエンドはGitHub Actions（`.github/workflows/notify.yml`）+ Supabase（無料枠）。**コードは実装済みだが、Supabaseプロジェクトの作成とGitHub Secretsの設定はユーザー側の手動作業が必要で未完了**（詳細は`README.md`のセットアップ手順を参照）。
