...
js/
  ...
  router.js       … 画面遷移関数の受け渡し（シートから「順位表へ」移動するための仲介・循環参照の回避）
  ...
  standings.js    … 順位表画面（球団マップ＋絞り込み・ア/ナ地区別テーブル・PS区分の色帯・ワイルドカード争い表・ゲーム差ものさし切替）
  postseason.js   … ポストシーズン区分（psClass）とワイルドカード争いカードの算出（既存APIレスポンスのみ使用、追加API呼び出しなし）
  gb-ruler.js     … 「ゲーム差ものさし」（地区内のゲーム差を横軸に並べる可視化。既存APIレスポンスのみ使用）
...
- **試合詳細**（UI見直し済み・2026-08-23）：スコアボード（ヘッダー・ラインスコア）を常にシート上部に固定し、結果サマリー・出場選手成績・打席ごとの結果はその下を独立スクロール。ラインスコアのイニング欄（例：8回裏）をタップすると、その回だけの打席結果をスコアボード直下に展開表示（再タップまたは×で閉じる）。シート上部のつまみ（ドラッグハンドル）を上へスワイプでフルスクリーン化、下へスワイプで閉じる（本文スクロールとの干渉を避けるため反応領域はつまみのみ）。試合に出場した選手全員の成績（打者は打席ごとの結果・打点、投手は登板イニング・球数・被安打・自責点・奪三振・与四死球）、勝敗投手・セーブ・本塁打のサマリー、打席ごとの結果を日本語の説明文で表示（`js/game-sheet.js`）。会場名はGoogleマップへのリンクになっている。
...
- **チーム紹介の全画面化**（2026-09-04）：下から出るボトムシートをやめ、画面いっぱいの全画面表示に変更（`css/card-shop-features.css`の`.sheet.full-sheet`）。上部に固定の操作バーを置き、「元に戻る」（呼び出し元のシートへ戻る／なければ閉じる）と「順位表へ」（シートを閉じて順位表画面へ移動）を追加。見出しは金箔フレームのカードにし、英語表記（`teams.js`の`enCity`/`enNick`と`DIVISIONS_EN`）＋日本語表記＋創設/WS制覇/リーグ優勝の3項目で構成する。シートから画面遷移する場合は`js/router.js`の`goToRoute`を使う（`app.js`を直接importすると循環参照になるため）。
- **ゲーム差ものさし**（2026-09-04）：順位表の絞り込みバーに「ゲーム差ものさし」トグルを追加。ONにすると各地区テーブルの上に、首位を左端としたゲーム差の横軸上に球団のペナントを並べる図（`js/gb-ruler.js`）を表示する。目盛りの最大値は実データの最大ゲーム差からキリの良い値を選ぶ。印は`--gb-mark-w`（印の幅）を差し引いた範囲に置くため、ゲーム差が最大の球団でも枠外にはみ出さない。追加のAPI呼び出しはなく、`getStandings`のレスポンスだけで描画する。
