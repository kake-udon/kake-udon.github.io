...
## 未実装（次のロードマップ）
2026-08-23時点で合意した次期ロードマップ（優先順）。各項目は既存のスクリーン・モジュール構成（`render〇〇(container)`パターン、`cachedFetch`、JSTユーティリティ、ボトムシート戻り先スタック）を踏襲して実装すること。新規のナビゲーションタブは追加せず、既存画面内へのタブ/セクション追加で実現する方針。

- **ロードマップ8：個人成績ランキング**（順位表画面に組み込み）
  - `js/standings.js`にリーグ順位表と並ぶ形で「個人成績」サブタブを追加し、打率・本塁打・打点・防御率・奪三振などのリーダーボード（上位10〜20名程度）を表示する。
  - データソースは`/stats/leaders?leaderCategories=...&season=...&sportId=1`。`api.js`に`getStatLeaders(season, category)`のような関数を追加し、既存の`cachedFetch`パターンに従う。
  - 選手名タップで既存の選手詳細ボトムシート（`player-sheet.js`）を開く（`sheet-stack.js`経由）。
  - あわせて順位表画面に「ワイルドカード順位表」表示を追加する。standings APIのレスポンスに含まれる`wildCardGamesBack`等のフィールドを利用し、新規APIコールは不要。

- **ロードマップ9：試合体験の強化**
  - ホーム画面（`js/home.js`）：進行中（ステータスがLive）の試合カードを一定間隔（60秒以上）で自動再取得し、スコア・イニングを更新する。タブがバックグラウンドの間はポーリングを止める。
  - ホーム画面に「本日の先発投手」セクションを追加：当日全試合の`probablePitcher`情報（`getGamesForJstDate`が既に取得しているデータを流用、追加APIコール不要）を一覧表示。
  - ポストシーズン進出時のブラケット表示は優先度を下げ、シーズン終盤（9〜10月）に着手を検討する別枠の作業とする（`standingsTypes=postseason`等の調査が必要）。

- **ロードマップ10：選手のキャリア成績表示**（`js/player-sheet.js`拡張）
  - 現状は当該シーズンの成績のみのため、`getPlayerDetail`のhydrateパラメータに`type=[yearByYear]`を追加（または専用関数を新設）し、年度別の打撃/投手成績テーブルを選手詳細ボトムシートに追加する。
  - 故障者リスト（IL）在籍中の選手は、ロースター取得時の`status`情報を使って選手カード・選手シートにバッジ表示する（`team-sheet.js`のロースター一覧、`player-sheet.js`の両方）。

- **ロードマップ11：日本人選手のかんたん導線**（`js/player-search.js`拡張）
  - 新規画面は追加せず、選手検索画面に「日本人選手」クイックフィルターチップを追加し、`getAllPlayers`の結果から`person.nameFirstLast`や国籍情報でフィルタする一本の導線とする。

- **ドキュメント整備**：README.mdの「未実装（次のロードマップ）」節がCLAUDE.mdと乖離している（ルール解説を未実装のまま記載）ため、実装済み一覧をCLAUDE.mdと整合させ、今後はCLAUDE.mdを一次情報源としてREADMEから参照する形に整理する。
