...
js/
  ...
  standings.js    … 順位表画面（球団マップ＋絞り込み・ア/ナ地区別テーブル）
...
- **順位表**（ロードマップ1、UI見直し済み・2026-08-23）：画面最上部に全球団の簡易US地図プロット（旧「チーム検索」画面を吸収）。リーグ・地区・PS圏内のみの絞り込み（何も選ばなければ全30球団を表示、条件はAND）を備え、地図の球団タップでチームシート（お気に入り登録可）を開く。PS圏内判定は`teamRecord.wildCardGamesBack`の符号で行う（`hasWildcard`は全球団trueになる構造的フラグのため使用不可、詳細は`standings.js`のコメント参照）。地図の下にア・リーグ/ナ・リーグタブ・地区別テーブル（勝敗・勝率・ゲーム差）、チーム名タップで直近±6日間の試合カレンダー（ボトムシート）。旧チーム検索の検索ボックス・地区別簡易リストは重複のため廃止。
