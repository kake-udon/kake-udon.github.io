...
- **送信時刻について**：GitHub Actionsのスケジュール実行は混雑時に数時間〜半日遅延することがある（実測で深夜〜翌朝に起動した回があった）。そのため18時台〜20時台に1日6回起動し、`scripts/send-notifications.mjs`がJSTの送信ウィンドウ（既定18:00〜20:59）内かどうかを判定して、ウィンドウ外に遅延した回は送信せずスキップする。ウィンドウを何度も逃した日は通知が届かないが、深夜・翌朝に通知が届いたり、日付をまたいだ実行のせいで翌日分が丸ごとスキップされたりするより望ましいという判断。ウィンドウは環境変数`NOTIFY_WINDOW_START_HOUR`/`NOTIFY_WINDOW_END_HOUR`で変更できる。
