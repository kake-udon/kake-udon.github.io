...
[`CLAUDE.md`](CLAUDE.md)の「次のロードマップ」、実装済みの内容と実データでの確認が残っている
項目は同「ロードマップ（2026ポストシーズン対応）」を参照してください。

## Web Push通知（毎日15時台のダイジェスト）のセットアップ

ロードマップ6として、毎日15時台（JST）にお気に入りチームの当日結果・次戦予定をプッシュ通知するダイジェスト機能を実装済みです。ただしGitHub Pagesは静的ホスティングのため、購読情報の保存と送信トリガーには外部サービス（Supabase / GitHub Actions）が必要です。**以下の設定は初回のみ、リポジトリのオーナーが手動で行ってください。**
...
- **送信時刻について**：MLBの試合は日本時間の朝〜昼に終わる（レギュラーシーズンで最も遅い西海岸のナイターでも14:30頃、ポストシーズンはさらに早い）ため、結果が出そろう**15:00〜17:59 JST**を送信ウィンドウにしている。レギュラーシーズンとポストシーズンで時間帯は分けていない。
- **遅延対策**：GitHub Actionsのスケジュール実行は混雑時に数時間〜半日遅延することがある（実測で深夜〜翌朝に起動した回があった）。そのため15:05〜17:35 JSTに1日6回起動し、`scripts/send-notifications.mjs`が送信ウィンドウ内かどうかを判定して、ウィンドウ外に遅延した回は送信せずスキップする。ウィンドウを何度も逃した日は通知が届かないが、深夜・翌朝に通知が届いたり、日付をまたいだ実行のせいで翌日分が丸ごとスキップされたりするより望ましいという判断。ウィンドウは環境変数`NOTIFY_WINDOW_START_HOUR`/`NOTIFY_WINDOW_END_HOUR`で変更できる（cronの時間帯も合わせて調整すること）。
