...
2026シーズンのポストシーズン対応（フェーズA〜E）はすべて実装済みです。この先の開発予定は
[`CLAUDE.md`](CLAUDE.md)の「次のロードマップ（フェーズF〜I）」、実装済みの内容は同
「ロードマップ（2026ポストシーズン対応）」を参照してください。

## Web Push通知（毎日15時以降のダイジェスト）のセットアップ

ロードマップ6として、毎日15時以降（JST）にお気に入りチームの当日結果・次戦予定をプッシュ通知するダイジェスト機能を実装済みです。ただしGitHub Pagesは静的ホスティングのため、購読情報の保存と送信トリガーには外部サービス（Supabase / GitHub Actions）が必要です。**以下の設定は初回のみ、リポジトリのオーナーが手動で行ってください。**
...
- **送信時刻について**：MLBの試合は日本時間の朝〜昼に終わる（レギュラーシーズンで最も遅い西海岸のナイターでも14:30頃、ポストシーズンはさらに早い）ため、結果が出そろう15時以降、同じ日の**15:00〜23:59 JST**を送信ウィンドウにしている。レギュラーシーズンとポストシーズンで時間帯は分けていない。
- **遅延対策**：GitHub Actionsのスケジュール実行は混雑時に数時間〜半日遅延することがある（実測で深夜〜翌朝に起動した回があった）。そのため15:05〜23:35 JSTの毎時5分・35分に起動し、`scripts/send-notifications.mjs`が送信ウィンドウ内かどうかを判定して、日付をまたぐほど遅延した回は送信せずスキップする（深夜・翌朝に届く問題と、日付をまたいだ実行で翌日分が丸ごとスキップされる問題を防ぐため）。当初はウィンドウを15:00〜17:59にしていたが、実測では起動が毎日1回・19〜21時台にずれ込み、2026-09-04〜09-22は全回スキップで通知が届かなかったため、当日中なら遅れても送る方針に改めた。スキップ時と送信結果はActionsの実行画面に注記（warning/notice）として表示される。ウィンドウは環境変数`NOTIFY_WINDOW_START_HOUR`/`NOTIFY_WINDOW_END_HOUR`で変更できる（cronの時間帯も合わせて調整すること）。
- **ポストシーズン中の内容**：お気に入りチームに本日の試合も今後の予定も無い場合はその行を出さず、全チームがそうなった購読者にはその日の通知を送らない（敗退後に中身のない通知を毎日送らないため）。
